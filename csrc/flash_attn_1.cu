...
template <typename T>
__global__ void
flash_attention_1_kernel(const T *Q, const T *K, const T *V, T *O, T *l, T *m, unsigned int Tr,
                         unsigned int Tc, unsigned int Br, unsigned int Bc, unsigned int num_heads,
                         unsigned int seq_len, unsigned int head_dim, T softmax_scale) {
  ...
  for (int j = 0; j < Tc; j++) {
    ...
    for (int i = j; i < Tr; i++) {
      ...
      T li_new = static_cast<T>(__expf(mi - mi_new)) * li +
                 static_cast<T>(__expf(cur_m_ij - mi_new)) * cur_l_ij;
      ...
      for (int d = 0; d < head_dim; d++) {
        ...
        O[Oi_offset + threadIdx.x * head_dim + d] =
            (static_cast<T>(1.0f) / li_new) *
            (li * alpha * O[Oi_offset + threadIdx.x * head_dim + d] + beta * pv);
      }
...
// launch
template <typename T>
void launch_flash_attention_01(const T *Q, const T *K, const T *V, T *O, unsigned int batch_size,
  ...
  cudaFuncSetAttribute(flash_attention_1_kernel<T>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                       sram_size);

  flash_attention_1_kernel<T><<<grid, block, sram_size>>>(
      Q, K, V, O, d_l_ptr, d_m_ptr, Tr, Tc, Br, Bc, num_heads, seq_len, head_dim, softmax_scale);
}
...
template void launch_flash_attention_01<__half>(const __half *Q, const __half *K, const __half *V,
                                                __half *O, unsigned int batch_size,
                                                unsigned int num_heads, unsigned int seq_len,
