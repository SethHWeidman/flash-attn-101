...
template <typename T>
__global__ void
flash_attention_2_kernel(const T *Q, const T *K, const T *V, T *O, T *L, unsigned int Tr,
  ...
  for (int j = 0; j <= block_id; j++) {
    ...
    for (int d = 0; d < head_dim; d++) {
      ...
      O[Oi_offset + threadIdx.x * head_dim + d] =
          alpha * O[Oi_offset + threadIdx.x * head_dim + d] + pv;
    }
  }
  // Oi = (1 / li) * Oi
  // Li = mi + log(li)
...
// launch
template <typename T>
void launch_flash_attention_02(const T *Q, const T *K, const T *V, T *O, unsigned int batch_size,
  ...
  cudaFuncSetAttribute(flash_attention_2_kernel<T>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                       sram_size);

  flash_attention_2_kernel<T><<<grid, block, sram_size>>>(
      Q, K, V, O, d_L_ptr, Tr, Tc, Br, Bc, num_heads, seq_len, head_dim, softmax_scale);
}
...
template void launch_flash_attention_02<__half>(const __half *Q, const __half *K, const __half *V,
