...
void print_device_info() {
  ...
  float const global_memory_size{
      static_cast<float>(device_prop.totalGlobalMem / (1 << 30))}; // 1 << 30 bytes = 1 GiB ~ 1 GB
  std::cout << "Global memory size: " << global_memory_size << " GB" << std::endl;

  // print peak memory bandwidth
  float const peak_bandwidth{
      static_cast<float>((2.0f * device_prop.memoryClockRate * device_prop.memoryBusWidth / 8) /
                         1.0e6)}; // 1.0e6 from kHz to Ghz
  std::cout << "Peak memory bandwidth: " << peak_bandwidth << " GB/s" << std::endl;

  std::cout << std::endl;
}

float measure_performance(std::function<void(cudaStream_t)> launcher, cudaStream_t stream,
                          const int num_warmups, const int num_repeats) {
