...
template <typename T>
__global__ void naive_gemm(const T *A, const T *B, T *C, T a, T b, unsigned int M, unsigned int N,
                           unsigned int K) {
...
template <typename T> __global__ void naive_softmax(T *input, T *output, unsigned int N) {
  // 3-pass softmax
  int idx = threadIdx.x + blockDim.x * blockIdx.x;
...
// QK[M, M] @ V[M, N]
template <typename T>
__global__ void naive_pv(const T *P, const T *V, T *O, unsigned int M, unsigned int N) {
  int idx = threadIdx.x + blockDim.x * blockIdx.x;
...
// launch
template <typename T>
void launch_naive_attention(const T *Q, const T *K, const T *V, T *O, unsigned int batch_size,
  ...
  naive_gemm<T>
      <<<qk_grid, qk_block, 0, stream>>>(Q, K, d_sm_ptr, sm_scale, 0, seq_len, seq_len, head_dim);
...
template void launch_naive_attention<half>(const half *Q, const half *K, const half *V, half *O,
                                           unsigned batch_size, unsigned int num_heads,
