...
int main() {
  ...
  const std::vector<std::pair<
      std::string,
      std::function<void(const __half *, const __half *, const __half *, __half *, unsigned int,
                         unsigned int, unsigned int, unsigned int, cudaStream_t stream)>>>
      attention_launchers = {
  ...
  for (const auto &[name, attention_launcher] : attention_launchers) {
    ...
    std::pair<float, float> results =
        profile_attention<__half>(batch_size, num_heads, seq_len, head_dim, attention_launcher,
                                  num_warmups, num_repeats, abs_tol, rel_tol);
    std::cout << std::endl;
  }
